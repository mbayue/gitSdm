...
export function useForceSync({
  ...
  }, [layoutType, selectedNodeId, focusedFilePath, nodes, forceGraphRef, forceInitialViewDoneRef]);
  ...
  useEffect(() => {
    ...
    if (action === 'zoomIn') {
      ...
      if (typeof currentZoom === 'number') ref.zoom(currentZoom * 1.3, 200);
    } else if (action === 'zoomOut') {
      const currentZoom = ref.zoom();
      if (typeof currentZoom === 'number') ref.zoom(currentZoom * 0.7, 200);
