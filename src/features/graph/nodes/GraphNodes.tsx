...
function CircleTreeNode({ id, data, selected, type }: NodeProps) {
  ...
  // Support Blast Radius inline style coloring
  const { blastRadiusActive, highlightedNodeIds } = useVizStore();
  const isHighlightedNeighbor = blastRadiusActive && highlightedNodeIds.has(id);

  let finalBorderColor: string | undefined = undefined;
  let finalBoxShadow: string | undefined = undefined;

  if (blastRadiusActive) {
    if (selected) {
      finalBorderColor = '#22d3ee';
      finalBoxShadow = '0 0 20px rgba(34,211,238,0.55)';
    } else if (isHighlightedNeighbor) {
      finalBorderColor = '#0891b2';
      finalBoxShadow = '0 0 14px rgba(8,145,178,0.35)';
    } else {
      finalBorderColor = diffStyles.borderColor || undefined;
      finalBoxShadow = diffStyles.boxShadow || undefined;
    }
  } else {
    finalBorderColor = selected ? color : (diffStyles.borderColor || undefined);
    finalBoxShadow = selected ? `0 0 14px ${color}22` : (diffStyles.boxShadow || undefined);
  }
