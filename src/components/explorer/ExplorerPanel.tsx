...
export function ExplorerPanel({ analysis, selectedFilePath, onSelectFile }: ExplorerPanelProps) {
  const { explorerOpen, setExplorerOpen, inspectorOpen, setInspectorOpen, focusedFilePath } = useVizStore();
  const rootLabel = analysis.meta.fullName.split('/')[1] ?? analysis.meta.repo;

  if (!explorerOpen) {
    return (
      <div className="flex w-10 shrink-0 flex-col items-center border-r border-white/[0.06] bg-zinc-950 py-2">
        <button
          type="button"
  ...
  return (
    <div className="flex h-full w-[220px] shrink-0 flex-col border-r border-white/[0.06] bg-zinc-950">
