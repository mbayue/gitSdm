...
export function DependencyHealthTab({ analysis }: DependencyHealthTabProps) {
  const query = useRepoHealth(analysis.meta.owner, analysis.meta.repo, analysis.meta.sha, true);
      ...
      <div className="space-y-4">
        <div className="rounded-lg border border-border bg-background p-4 text-center space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            {query.isError ? 'Dependency health could not be loaded.' : 'Loading dependency health…'}
          </p>
          {query.isError && (
            <button onClick={() => void query.refetch()} className="text-accent">
              Try again
            </button>
          )}
        </div>
    ...
    <div className="space-y-5 pb-6">
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          ...
          >
            <div className="text-[16px] font-mono text-destructive font-semibold leading-none mb-1">
              {summary.unknown + summary.errors}
            </div>
          ...
          >
            <div className="text-[16px] font-mono text-muted-foreground font-semibold leading-none mb-1">
              {summary.unsupported}
            </div>
      ...
      <div className="rounded-md border border-border bg-background px-3 py-2 flex items-start gap-2">
        ...
        <p className="text-xs text-muted-foreground leading-snug">
          Freshness checks currently support <strong>npm</strong> only. Other ecosystems are inventoried but versions
          remain unchecked.
        </p>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          <h3 className="text-xs font-bold text-foreground uppercase tracking-widest">Dependencies</h3>
                      ...
                      <div className="grid grid-cols-[80px_1fr] gap-1 text-xs">
                        ...
                        <div className="text-foreground font-mono space-y-1">
                          {item.packageNames
                            ...
                            .map((name, i) => (
                              <TooltipHint key={i} content={name}>
