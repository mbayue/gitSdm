...
const getScoreColor = (val: number) => {
  if (val >= 80) return { text: 'text-success', bg: 'bg-emerald-500' };
  if (val >= 60) return { text: 'text-warning', bg: 'bg-amber-500' };
  return { text: 'text-destructive', bg: 'bg-rose-500' };
};
...
function RiskCard({
  ...
  return (
    <div className="rounded-md border border-border bg-card p-3 space-y-2 hover:border-ring/50 transition-all group">
      <div className="flex items-start justify-between gap-2">
        <span className="font-semibold text-foreground text-xs leading-snug">{s.title}</span>
        <span className={cn(
          "text-xs font-bold uppercase px-1.5 py-0.5 rounded border shrink-0 font-mono",
          s.risk === 'high' ? "text-destructive bg-rose-500/10 border-rose-500/20" :
          s.risk === 'medium' ? "text-warning bg-amber-500/10 border-amber-500/20" :
          "text-success bg-emerald-500/10 border-emerald-500/20"
        )}>
          {s.risk}
        </span>
      </div>
      <div className={cn("text-xs text-muted-foreground leading-relaxed font-sans", !expanded && "line-clamp-2")}>
        {s.description}
      </div>
      {s.files && s.files.length > 0 && expanded && (
        <div className="pt-2 border-t border-border mt-2">
          <span className="text-xs font-mono text-muted-foreground uppercase tracking-widest mb-1 block">Affected Files</span>
          <div className="flex flex-wrap gap-1">
            {s.files.map((f: string) => (
              <button
                ...
                className="text-xs font-mono text-accent bg-accent/10 hover:bg-accent/20 px-1.5 py-0.5 rounded transition-colors"
              >
                {f}
              </button>
            ))}
        ...
        {aiSubTab === 'health' ? (
          <div className="space-y-4">
            {healthSubMode === 'audit' ? (
              <>
                  ...
                  <div className="space-y-2">
                    <div className="grid grid-cols-3 gap-2">
                      {firstRow.map((dim) => {
                        ...
                        const Icon = dim.icon;
                        return (
                          <div key={dim.key} className="bg-card border border-border rounded-md p-2.5 flex flex-col justify-between min-h-[70px] transition-all hover:border-ring/50">
                            <div className="flex items-center gap-1.5">
                              <Icon className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground truncate">{dim.label}</span>
                            </div>
                            <div className="mt-1">
                ...
                {healthData?.summary && (
                  <div className="flex items-start gap-1.5 text-xs italic text-muted-foreground mt-3 pl-1 leading-normal border-l-2 border-accent/40">
              ...
              <>
                {refactorData?.suggestions && refactorData.suggestions.length > 0 ? (
                  ...
                  <div className="text-muted-foreground italic py-4 text-center text-xs">Run a diagnosis to identify codebase risks.</div>
                )}
              </>
            )}
          </div>
          ...
          <div className="space-y-4">
            {activePlayground === 'roast' && (
              roastData ? <Markdownish text={roastData.roast} /> : <div className="text-muted-foreground italic py-4 text-center text-xs">Ready to get roasted? Select Repo Roast below.</div>
            )}
            {activePlayground === 'readme' && (
              readmeEnhanceData ? (
                <Markdownish text={readmeEnhanceData.readme} />
              ) : <div className="text-muted-foreground italic py-4 text-center text-xs">Ready to enhance your README? Select Readme Enhancer below.</div>
            )}
          </div>
        ) : (
          currentExplanation ? (
