...
const GraphCanvas = lazy(() =>
  import('@/features/graph/canvas/GraphCanvas').then((m) => ({
    default: m.GraphCanvas,
  })),
);

const ArchitectureView = lazy(() =>
  import('@/components/viz/ArchitectureView').then((m) => ({
...
export function VizPage() {
  ...
  useEffect(() => {
    if (churnError) setToastMessage('Churn history could not be loaded. Check GitHub access or try again later.');
    else if (churnLoading) setToastMessage('Loading churn history…');
  }, [churnError, churnLoading, setToastMessage]);

  // Code Inspector state & auto-open helpers
  const { codeInspectorState, setCodeInspectorState, setPreferredOpenState } = useCodeInspectorState(focusedFilePath);
  ...
  const panelWidths = fitWorkspacePanels(
    viewportWidth - minibarWidth,
    explorerOpen ? leftWidth : 0,
    aiSidebarOpen ? rightWidth : 0,
  );
  useEffect(() => {
  ...
  const handleSelectFile = useCallback(
    (path: string) => {
      const node = (combinedGraph || data?.graph)?.nodes.find(
        (item) => item.type === 'file' && item.data.path === path,
      );
  ...
  if (error) {
    return <VizError error={error instanceof Error ? error.message : String(error)} />;
  }
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex min-h-0 flex-1 overflow-hidden relative">
            ...
            >
              <ExplorerPanel analysis={data} selectedFilePath={selectedFilePath} onSelectFile={handleSelectFile} />
            </VizSidebar>
            ...
            >
              {activeView === 'graph' && colorMode === 'churn' && (
                <ChurnStatus owner={owner} repo={repo} sha={data.meta.sha} />
              )}
                ...
                {activeView === 'architecture' && (
                  <Suspense
                    fallback={
                      <div
                        role="status"
                        className="flex h-full w-full items-center justify-center gap-2 bg-background text-sm text-muted-foreground"
                      >
            ...
            >
              <AISidebar analysis={data} graphDiff={graphDiff} />
            </VizSidebar>
          </div>
        ) : null}
      </div>

      <BottomStatusBar analysis={data} showMinimap={showMinimap} activeView={activeView} />
        ...
        {toastMessage && (
          ...
          >
            <Card role="status" className="flex items-center gap-2.5 border-border bg-card px-4 py-3 shadow-xl">
              ...
              <div>
                <p className="text-sm text-foreground break-words max-w-[280px] leading-relaxed">{toastMessage}</p>
              </div>
            </Card>
          </motion.div>
        )}
