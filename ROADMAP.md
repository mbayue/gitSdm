...
- **Custom OpenAI-Compatible AI Endpoints** — Allows configuring custom models and public HTTPS endpoints in Settings with SSRF DNS pinning and private-address protections.
- **Decoupled Search Embedding Configuration** — Independent configuration of embedding models, credentials, and endpoints isolated from chat provider settings.
- **Resilient AI Task Lifecycle & Cache Invalidation** — Task-level promise sharing across panel remounts and automatic cache invalidation on settings changes.
