...
export async function handleApiRequest(
  ...
  try {
    ...
    if (pathname === '/api/repo/file' && method === 'GET') {
      ...
      const file = await getRepoFileContent(q.data.owner, q.data.repo, q.data.path);
    ...
    if (pathname === '/api/ai/architecture' && method === 'POST') {
      ...
      const result = await explainArchitecture(parsed.data.owner, parsed.data.repo);
      sendJson(res, 200, result);
      return true;
    }
    ...
    if (pathname === '/api/ai/suggest-files' && method === 'POST') {
      ...
      const result = await suggestFiles(parsed.data.owner, parsed.data.repo);
      sendJson(res, 200, result);
      return true;
    }
